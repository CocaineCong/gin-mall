...
func (service *FavoritesService) Show(id uint) serializer.Response {
	...
	if err := dao.DB.Model(&favorites).Preload("User").
	...
	err := dao.DB.Model(model.User{}).Preload("User").Where("user_id=?", id).
...
func (service *FavoritesService) Create(id uint) serializer.Response {
	...
	code := e.SUCCESS
	dao.DB.Where("user_id=? AND product_id=?", id, service.ProductID).Find(&favorite)
	dao.DB.Model(model.User{}).First(&user, id)
	dao.DB.Model(model.User{}).First(&boss, service.BossID)
	dao.DB.Model(model.Product{}).First(&product, service.ProductID)
...
func (service *FavoritesService) Delete(uid uint, pid string) serializer.Response {
	var favorite model.Favorite
	code := e.SUCCESS
	err := dao.DB.Where("user_id=? AND product_id=?", uid, pid).Find(&favorite).Error
	if err != nil {
		logging.Info(err)
		code = e.ErrorDatabase
		return serializer.Response{
	...
	err = dao.DB.Delete(&favorite).Error
	if err != nil {
		logging.Info(err)
		code = e.ErrorDatabase
		return serializer.Response{
